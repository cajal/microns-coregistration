...
def create_grid(um_sizes, desired_res=1):
    ...
    # Create grid
    out_sizes = [int(np.round(um_s / res)) for um_s, res in zip(um_sizes, desired_res)]
...
def sample_grid(volume, grid, mode='bilinear'):
    """ 
    ...
    # Resample
    resampled = F.grid_sample(volume[None, None, ...], norm_grid[None, None, ...], padding_mode='zeros', mode=mode, align_corners=True)
...
def get_affine_grid(um_sizes, reg_params_dict, desired_res=1):
    """ Get registered grid for this registration. """

    # Create grid at desired resolution
    grid = create_grid(um_sizes, desired_res=desired_res)  # h x w x 2
    grid = torch.as_tensor(grid, dtype=torch.double)

    # Apply required transform
    a11 = reg_params_dict['a11']
    a21 = reg_params_dict['a21']
    a31 = reg_params_dict['a31']
    a12 = reg_params_dict['a12']
    a22 = reg_params_dict['a22']
    a32 = reg_params_dict['a32']
    reg_x = reg_params_dict['reg_x']
    reg_y = reg_params_dict['reg_y']
    reg_z = reg_params_dict['reg_z']
    linear = torch.tensor([[a11, a12], [a21, a22], [a31, a32]], dtype=torch.double)
    translation = torch.tensor([reg_x, reg_y, reg_z], dtype=torch.double)
